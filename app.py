    ...
    with app.app_context():
        ...
        data = {
            "movies": [{
                ...
                "description": "США после Гражданской войны. Легендарный охотник за головами Джон Рут по кличке Вешатель "
                               ...
                               "пестрое общество: генерал конфедератов, мексиканец, ковбой… И один из них - не тот, "
                               "за кого себя выдает.",
                ...
                "description": "Эксцентричный охотник за головами, также известный как Дантист, "
                               "промышляет отстрелом самых опасных преступников. Работенка пыльная, и "
                               "без надежного помощника ему не обойтись. Но как найти такого и желательно "
        ...
        for movie in data["movies"]:
            ...
            with db.session.begin():
                if db.session.query(Movie).filter(Movie.id == movie["pk"]).first() is None:
                    db.session.add(m)
        ...
        for director in data["directors"]:
            ...
            with db.session.begin():
                if db.session.query(Director).filter(Director.id == director["pk"]).first() is None:
                    db.session.add(d)
